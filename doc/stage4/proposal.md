...
| Q7  | 多语言检索实现                                    | FTS5 trigram 为主（B3 实测中文/日文/英文/一般子串）；1–2 字符短查询/特殊 URL/FTS 不可用 → 参数化精确匹配/LIKE 安全降级；排序确定性（精确>前缀>tag/group>name/domain>note，priority 有限加分，recency 次级 tie-break） |
