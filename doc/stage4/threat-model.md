...
场景仍为 **NOT RUN**（2026-08-16 真实验收执行两轮分别止于场景 1b 与 4c——
两处夹具缺陷已最小修复并离线全绿（1b URL 同 origin 断言校准/s4c 定位文案），
观察场景未达执行；不承诺、不冒充）；语义层残余风险（§5 六类）维持「不承诺」
登记不变。2026-08-16 新增观察性登记（不影响机器断言）：disabled 条目对
agent 检索不可见（search/list 候选 SQL 过滤 `deleted_at IS NULL`，契约
语义）——自然语言「恢复」任务在模型侧需用户提供定位信息，属计划内语义
边界。
