...
| IPC/bridge 扩展          | shared/types/ipc.ts + preload                      | sources 通道白名单（sender+主帧校验、逐参数验证、事件只发主窗口）；quick-add 无参数（main 读活动 Tab）；两阶段 hard-delete                                                                                                                                         | B5 ✅                                                              |
...
```
模型调用 source_search(query≤500) / source_list / source_get（B4 适配器硬编码 audience='agent'）
→ 本地检索（B3）：查询归一化（trim/NFC/控制字符剔除）→ 分流（决议 #60：1 字符精确 /
   2 字符精确+前缀+子串 LIKE / ≥3 字符 FTS5 trigram + 精确/前缀/标签/分组候选 / URL 查询
   精确+前缀；候选有界 200）→ FTS 不可用仅指建库后 MATCH/构造失败 → 参数化降级（决议 #62）
→ 分享模式过滤（audience='agent'：blocked 不可见；metadata 零 note 字节且 note 不参与命中
   排序；full 命中附 ≤200 摘录 + provenance）+ 返回 allowlist
→ 确定性排序（决议 #61：档位严格不可跨档——精确>前缀>tag/group>name/domain>note；
   priority/recency 仅同档内；lastUsedAt=null 末位；scope+canonicalKey+id 收尾全序）
→ 命中 ≤10（search）/每页 ≤20（list）
→ ToolResult 预算截断 → UNTRUSTED_TOOL_RESULT 块回注模型（B4）
→ 审计（查询串全量 ≤500 / 分页参数与条数，B4）
```
