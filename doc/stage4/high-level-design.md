...
| FTS 索引维护             | src/main/sources/repository/source-search-index.ts | FTS5 表与主表**事务内写同步落 B2**（Repository 显式语句，决议 #54：FTS 行 = 非 hard-deleted 行镜像、查询期过滤归 B3）；查询串纯函数构造/排序/rebuild 落 B3  | B2/B3    |
