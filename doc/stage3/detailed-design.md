...
- 真实 Provider 可选验证（A7，需用户提供 Key，询问边界）：真实 tool calling
  ...
  **RT-10 验收含义按三类诚实边界校准（2026-08-14，threat-model §4 同步）**：
  机器可验证（程序边界对诱导式提议的强制阻断：未知工具/非法参数/L2・L3/
  documentId・allowedKind/确认门/执行器复核）/ 观察性（真实模型在本次固定网页与
  固定任务中是否遵守「不把网页文字当指令」，如实记录不推广）/ 不保证（模型永远
  不被诱导产出合法 L0/L1 参数，按 §5 残余风险登记）；真实测试失败时不得放宽权限、
  自动确认或修改红队夹具制造通过。
