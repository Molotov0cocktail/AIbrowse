...
harness 扩展 + 真 Key 零暴露扫描）——**真实执行需用户另行授权，未授权
NOT RUN 且不阻塞离线 C9**。
...
- **做**：8.20 红队矩阵（FRT-01～FRT-12 每项独立断言，落点 threat-model
  ...
  不提交 Git）。
- **不做**：真实 Provider 实际执行（未授权不联网调用付费 API）；修改
  产品契约制造红队通过；放宽任何验收标准。
...
4. 真实 Provider 执行：**仅当用户另行授权**（一次完整运行 + 台账；沿用
   harness DPAPI 流程；未经授权绝对不发起付费/公网请求）。

## 验收标准

- FRT-01～FRT-12 全部「机器可证明」（dev+生产退出码 0 + threat-model
  §4.1 证据回填）；观察性子集如实登记不冒充；
- 端到端离线场景覆盖 Fifth §7 全部七条映射（设计映射表记录）；
- 隐私扫描逐通道零命中（随机标记）；门控互斥实测；无 Key 路由退出码 0
  - 真实请求 0；
- 真实 Provider 维度：授权后执行并台账，未授权 NOT RUN（不阻塞离线 C9，
  计入 C10 判定缺口）。
...
- FRT 红队发现产品缺陷 → 先测后修独立提交（同 SRT-08 先例），修复归
  对应任务闭环或新修复任务，不迁就实现；
- 真实 Provider 未授权 → NOT RUN 如实登记（不得以 FakeProvider 替代、
  不得冒充历史证据）；
