...
- **做**：两组候选 tarball/许可证/维护/供应链/Node24/Electron 构建核验；XML/HTML 敌手与兼容语料；
  Node 核心 http/https 连接时 lookup；NetworkPolicy/RobotsPolicy；SAX Feed Discovery；FeedProjection；
...
1. **资格门**：只读/临时隔离核验两组 package、许可证、依赖、构建；XML 运行 DTD/XXE/bomb/depth/
   encoding/RSS/Atom；HTML 运行 malformed/depth/node/attribute/script/iframe/subresource/cookie corpus。
...
- WRT-01～WRT-08/WRT-19 对应项聚焦全绿；每跳 scheme/host/IP/DNS/downgrade 独立断言。
- RSS2/Atom/namespaces/CDATA/encoding/重复 identity/304/ETag/Last-Modified 有稳定 oracle。
- DTD/entity/XInclude、2 MiB/深度/字段/节点/时间预算 fail-closed，零假 Projection。
