...
function verifyContent(proposal: EvidenceProposal, content: CaptureContent): ContentCheckResult {
  ...
  if (proposal.type === 'table-cell') {
    ...
    // Adjudication #173 empty cell fail-closed: after the trusted realCell is
    // read and BEFORE proposal matching, realCell === '' (explicitly empty on
    // the page / normalized empty after control/bidi stripping / normalize
    // short-row padding) is always value-invalid — no excerpt/value
    // combination (including both null) may assemble a VerifiedEvidence; hash
    // coverage does not distinguish explicit empty from synthetic padding.
    if (real === '') return bad('value-invalid', locator);
    // value/excerpt must exactly equal the trusted real value (checked separately)
    ...
    // The header is generated by the program from the real table header; a
    // non-empty proposal header must equal the real header. A normalized empty
    // header placeholder '' (empty/whitespace header) maps to null in the
    // Evidence output.
    const rawHeader =
